...
class Robot:
    ...
    def display_image(self, filename):
        msg_body = { "filename" : filename }
        response = requests.post('http://' + self.ip + '/api/images/display', params = msg_body).json()
        if response["status"] == "Success":
            return response["result"] 
        elif response["status"] == "Failed":
            print("Error changing facial expression: ", response["error"])
        else:
            print("Error: Failed to change facial expression.")

    def play_audio(self, filename):
        msg_body = { "filename" : filename }
        response = requests.post('http://' + self.ip + '/api/audio/play', params = msg_body).json()
        if response["status"] == "Success":
            return response["result"] 
        elif response["status"] == "Failed":
    ...
    def move_arms(self, arm, position, velocity):# , duration):
        msg_body = {"arm" : arm, "position" : position, "velocity" : velocity } #, "duration" : duration}
        response = requests.post('http://' + self.ip + '/api/arms', params = msg_body).json()
        if response['status'] == 'Success':
            return response['result']
        elif response['status'] == 'Failed':
            print("Error with moving arms: ", response['error'])
        else:
            print("Error : Failed to move arms.")

    def change_led(self, red, blue, green):
        msg_body = {"red" : red, "blue" : blue, "green" : green}
        response = requests.post("http://" + self.ip + '/api/led', params = msg_body).json()
        if response['status'] == 'Success':
            return response['result']
        elif response['status'] == 'Failed':
            print("Error changing LED: ", response['error'])
        else:
            print("Error: Failed to change LED")
